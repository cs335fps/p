...
void set_title(void)
{
	...
	XMapWindow(dpy, win);
	XStoreName(dpy, win, "CS371 - Lab OpenGL");
}
...
void render(void)
{
	...
	//
		...
		MakeVector(m1, 2.4f, -0.1f, qtrans);
	...
	ggprint8b(&r, 16, 0x0088ff88,
		(g.quadMovement) ? "<Q> Quad move (On)" : "<Q> Quad move (Off)");
	ggprint8b(&r, 16, 0x00aaaaaa,
		(g.shadows) ? "<S> Shadows (On)" : "<S> Shadows (Off)");
	ggprint8b(&r, 16, 0x0066aaff,
		(g.baseRotate) ? "<B> Base rotate (On)" : "<B> Base rotate (Off)");
	//
	glPopAttrib();
}
